...
class GenerateCharacters():
    ...
    def generate_random_characters_2d(self, num_rows, num_cols):
        if self.is_string:
            return [''.join(np.random.choice(list(self.character_set), num_cols).tolist()) for _ in range(num_rows)]
        return [np.random.choice(list(self.character_set), num_cols).tolist() for _ in range(num_rows)]
