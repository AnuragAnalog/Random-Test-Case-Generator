...
class GenerateIntegers:
    ...
    def generate_random_integers_range(self, start, end, num_integers):
        if self.sort:
            return sorted(
                np.random.choice(
                    range(start, end + 1), size=num_integers, replace=self.distinct
                ).tolist()
            )
        ...
        if self.sort:
            return np.sort(
                np.random.choice(
                    ...
                    replace=self.distinct,
                )
            ).tolist()
        return np.random.choice(
            range(start, end + 1), size=(num_rows, num_cols), replace=self.distinct
        ).tolist()
...
def generate_integers():
    ...
    if option == "Random integers":
        ...
        num_integers = st.number_input(
            "Number of integers to generate:", min_value=1, step=1
        )
        ...
        if st.button("Generate"):
            for _ in range(n):
                st.code(
                    gen_int.generate_random_integers_range(
                        start_range, end_range, num_integers
                    )
                )
        ...
        if st.button("Generate"):
            for _ in range(n):
                st.code(
                    gen_int.generate_random_integers_2d_range(
