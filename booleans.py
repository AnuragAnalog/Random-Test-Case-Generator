...
class GenerateBooleans():
    ...
    def generate_random_booleans_2d(self, num_rows, num_cols):
        if self.binarise:
            return [np.random.choice([True, False], num_cols).astype(int).tolist() for _ in range(num_rows)]
        return [np.random.choice([True, False], num_cols).tolist() for _ in range(num_rows)]
